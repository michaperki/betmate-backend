...
const badWagerDataWDL = {
  ...
  data: 1, // should be string
  move_number: 'one', // should be number
};
...
describe('Wager model validation', () => {
  ...
  it('blocks WDL wager with invalid domain values', async (done) => {
    try {
      ...
      // eslint-disable-next-line max-len
      expect(saveError.message).toBe('Wager validation failed: wdl: Cast to Boolean failed for value "wdl" at path "wdl", odds: Cast to Number failed for value "good" at path "odds", move_number: Cast to Number failed for value "one" at path "move_number", amount: Path `amount` (-10) is less than minimum allowed value (0).');
      done();
    } catch (error) {
      done(error);
    }
